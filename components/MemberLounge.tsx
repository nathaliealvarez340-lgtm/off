  ...
  return (
    <main className="member-lounge">
      <nav className="lounge-nav">
        ...
        <div>
          <a href="#archive">Biblioteca</a>
          <a href="#signals">Signals</a>
          <a href="#exclusives">Recursos</a>
      ...
      <Reveal className="lounge-section archive-section" id="archive">
        ...
        <div className="archive-volumes">
          {libraries.map((item) => (
            <article className="archive-volume" key={item.id}>
              <span>Colección {item.number ?? "—"}</span>
              <h3>{item.title}</h3>
              <p>{item.description}</p>
              <div className="lounge-linked-resources">{links(item).map((link) => <a href={link.url} key={link.url}>{link.label}</a>)}</div>
              {links(item)[0] ? <a href={links(item)[0].url}>Abrir volumen</a> : <em>Volumen editorial</em>}
            </article>
          ))}
          {!libraries.length ? <p className="lounge-empty">La Biblioteca todavía está tomando forma.</p> : null}
        </div>
      </Reveal>
      ...
      <Reveal className="lounge-section signals-section" id="signals">
        ...
        <div className="signals-list">
          {signals.map((signal, index) => (
            <article className="signal-note" key={signal.id}>
              <span>Signal #{signal.number ?? String(index + 1).padStart(3, "0")}</span>
              <p>{signal.content ?? signal.description}</p>
            </article>
          ))}
      ...
      <Reveal className="lounge-section early-access">
        ...
        <div className="early-editions">
          {earlyAccess.map((edition) => (
            <article key={edition.id}>
              <time>{edition.releaseDate ? <LocalDate value={edition.releaseDate} /> : edition.statusLabel}</time>
              <span>{edition.statusLabel}</span>
              <h3>{cleanText(edition.title)}</h3>
              <p>{cleanText(edition.description ?? edition.content ?? "")}</p>
            </article>
          ))}
          {!earlyAccess.length ? <p className="lounge-empty">La próxima edición todavía está tomando forma.</p> : null}
        </div>
      </Reveal>
