...
export function GalleryPostViewer({
  post,
  initialLanguage = "es",
  onClose,
  previousControl,
  ...
  useEffect(() => {
    ...
    if (!previewMode) {
      ...
      fetch(`/api/gallery?id=${encodeURIComponent(post.id)}`, {
        signal: controller.signal,
      })
        .then((response) => (response.ok ? response.json() : null))
        .then((data: { post?: GalleryPostData } | null) => {
          if (data?.post) {
  ...
  async function toggleLike() {
    ...
    try {
      ...
      setMessage(
        error instanceof Error ? error.message : copy.interactionError,
      );
    } finally {
      setLikePending(false);
    }
  ...
  async function recordShare(type: "EXTERNAL" | "COPY_LINK") {
    await fetch(`/api/gallery/${post.id}/share`, {
      method: "POST",
  ...
  async function copyLink() {
    try {
      await navigator.clipboard.writeText(
        new URL(post.permalink, window.location.origin).toString(),
      );
  ...
  return (
    <div
      className={`off-gallery-viewer ${standalone ? "is-standalone" : ""}`}
      role={standalone ? undefined : "dialog"}
      aria-modal={standalone ? undefined : "true"}
      aria-label={post.title || copy.gallery}
      tabIndex={-1}
    >
      {onClose ? (
        <button
          className="off-gallery-viewer-close"
          type="button"
          onClick={onClose}
      ...
      <div className="off-gallery-viewer-media">
        <div className="off-gallery-media-frame">
          {post.mediaType === "IMAGE" ? (
            <img
              src={post.mediaUrl}
              alt={post.altText || post.caption || copy.galleryImageAlt}
              style={mediaStyle}
            />
          ) : (
            <video
              ...
              preload="metadata"
              muted={Boolean(
                post.musicSource || post.audioUrl || post.spotifyTrackId,
              )}
              style={mediaStyle}
            />
          )}
      ...
      <aside className="off-gallery-viewer-panel">
          ...
          <>
            ...
            {shareOpen ? (
              <div className="off-gallery-share-sheet">
                <button type="button" onClick={shareExternal}>
                  <Share2 />
                  {copy.shareOtherApp}
                </button>
            ...
            <ThreadedComments
              endpoint={`/api/gallery/${post.id}/comments`}
              initialCount={post.commentCount}
              language={language}
              reportType="GALLERY_COMMENT"
              compact
              autoLoad
              staticHeader
            />
        ...
        {message ? (
          <p className="off-gallery-feedback" role="status">
            {message}
          </p>
        ) : null}
      </aside>
