...
export function memberOffId(memberNumber?: number | null) {
  return memberNumber
    ? `OFF #${String(memberNumber).padStart(4, "0")}`
    : "OFF Member";
}
...
const memberSelect = {
  ...
  achievements: {
    ...
    take: 1,
    select: { id: true, title: true, achievedAt: true },
  },
...
export async function syncMemberAchievements(userId: string) {
  ...
  if (user._count.completions >= 1)
    achievements.push({
      type: "FIRST_EDITION",
      title: "Completó su primera edición.",
      metadata: { completedCount: user._count.completions },
    });
  if (user._count.completions >= 5)
    achievements.push({
      type: "FIVE_ARTICLES",
      title: "Completó cinco capítulos de OFF.",
      metadata: { completedCount: user._count.completions },
    });
  if (Date.now() - user.createdAt.getTime() >= 365 * 24 * 60 * 60 * 1000)
    achievements.push({
      type: "FIRST_ANNIVERSARY",
      title: "Cumplió un año como miembro de OFF.",
    });

  await Promise.all(
    achievements.map((achievement) =>
      db.memberAchievement.upsert({
        where: { key: `${userId}:${achievement.type}` },
        create: {
          key: `${userId}:${achievement.type}`,
          userId,
          ...achievement,
        },
...
function serializeMember(
  ...
  return {
    ...
    connectionDirection:
      !connection || !viewerId
        ? null
        : connection.receiverId === viewerId
          ? "INCOMING"
          : "OUTGOING",
  };
}
...
export async function getCommunityMembers(
  ...
  const connections = await db.memberConnection.findMany({
    where: {
      pairKey: {
        in: members.map((member) => connectionPairKey(viewerId, member.id)),
      },
    },
    select: {
      pairKey: true,
  ...
  const connectionMap = new Map(
    connections.map((connection) => [connection.pairKey, connection]),
  );
  return members.map((member) =>
    serializeMember(
      member,
      connectionMap.get(connectionPairKey(viewerId, member.id)),
      viewerId,
    ),
  );
}

export async function getCommunityMemberProfile(
  viewerId: string,
  memberId: string,
) {
  const db = getDb();
  const blocked = await db.memberBlock.findFirst({
    where: {
      OR: [
  ...
  const [member, connection, posts, achievements, connectionCount] =
    await Promise.all([
      db.user.findFirst({
        where: { id: memberId, role: "USER" },
        select: memberSelect,
      }),
      db.memberConnection.findUnique({
        where: { pairKey: connectionPairKey(viewerId, memberId) },
      ...
      db.memberConnection.count({
        where: {
          status: "CONNECTED",
          OR: [{ requesterId: memberId }, { receiverId: memberId }],
        },
      }),
    ]);
  ...
  return {
    ...
    achievements: achievements.map((achievement) => ({
      ...
      reactionCount: achievement._count.reactions,
    })),
  };
}
...
function serializeComment(
  comment: CommunityPostRow["comments"][number],
  viewerId: string,
  canModerate: boolean,
): SocialCommentData {
  const serializeReply = (
    reply: CommunityPostRow["comments"][number]["replies"][number],
  ): SocialCommentData => ({
    id: reply.id,
    ...
    user: {
      ...
      offId: memberOffId(reply.user.memberProfile?.memberNumber),
    },
  ...
  return {
    ...
    user: {
      ...
      offId: memberOffId(comment.user.memberProfile?.memberNumber),
    },
...
export async function getCommunityFeed(
  ...
  const connectedIds = connectedRows.map((row) =>
    row.requesterId === viewerId ? row.receiverId : row.requesterId,
  );
  const blockedRows = await db.memberBlock.findMany({
    where: { OR: [{ blockerId: viewerId }, { blockedId: viewerId }] },
    select: { blockerId: true, blockedId: true },
  ...
  const achievementRows =
    tab === "conversations"
      ? []
      : await db.memberAchievement.findMany({
          ...
          take:
            tab === "achievements"
              ? limit + 1
              : Math.max(5, Math.ceil(limit / 3)),
        });

  const postItems: CommunityFeedItem[] = postRows
    .slice(0, limit)
    .map((post) => ({
      id: post.id,
      kind: "THOUGHT",
      ...
      user: {
        id: post.user.id,
        name: post.user.name,
        offId: memberOffId(post.user.memberProfile?.memberNumber),
        badges: earnedBadges(post.user._count.completions),
      },
      ...
      comments: post.comments.map((comment) =>
        serializeComment(comment, viewerId, role === "ADMIN"),
      ),
      canDelete: role === "ADMIN" || post.userId === viewerId,
    }));
  const achievementItems: CommunityFeedItem[] = achievementRows
    .slice(0, limit)
    .map((achievement) => ({
      id: achievement.id,
      kind: "ACHIEVEMENT",
      ...
      user: {
        ...
        offId: memberOffId(achievement.user.memberProfile?.memberNumber),
        badges: earnedBadges(achievement.user._count.completions),
      },
  ...
  const items = [...postItems, ...achievementItems]
    .sort((a, b) => {
      const connectionBoostA = connectedIds.includes(a.user.id) ? 1 : 0;
      const connectionBoostB = connectedIds.includes(b.user.id) ? 1 : 0;
      if (tab === "for-you" && connectionBoostA !== connectionBoostB)
        return connectionBoostB - connectionBoostA;
      return new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime();
    })
    .slice(0, limit);
  return {
    items,
    hasMore: postRows.length > limit || achievementRows.length > limit,
  };
}
